 ...
 * @link http://pageconfig.com/post/portable-utf8
 *
 * This library contains various UTF8 functions, first part taken from
 * http://noteslog.com/post/escaping-and-unescaping-utf-8-characters-in-php/
 * and second part is the portable-utf8 library
 * http://pageconfig.com/post/portable-utf8
 *
 * Used portable-utf8 library version was 1.3
 *
 * See data/doc/utf8 for more info
 *
...
function utf8_escape($string){
    try{
        ...
        throw new lsException('utf8_escape(): Failed for string "'.str_log($string).'"', $e);
    }
}
...
function utf8_unescape($string){
    try{
        ...
        throw new lsException('utf8_unescape(): Failed for string "'.str_log($string).'"', $e);
    }
}
...
/*
 * This object can do utf8 escaping, yay!
 */
class Zend_Utf8
{
    /**
    ...
    public static function escape($value, array $options = array())
    {
        ...
        {
            throw new lsException('Zend_Utf8::escape(): Expected a valid write (callable, array).');
        }
        ...
        for($i = 0; $i < $length; $i++) {
            ...
            switch (true) {
                ...
                default:
                    ...
                    throw new lsException('Zend_Utf8::escape(): Expected a valid UTF-8 character.');
                break;
            }
        }
    ...
    public static function utf8CharToCodePoint($utf8Char)
    {
        ...
        else
        {
            ...
            switch (strlen($utf8Char))
            {
                ...
                default:
                    //no more cases in unicode, whose range is 0x00000000 - 0x0010FFFF
                    throw new lsException('Zend_Utf8::utf8CharToCodePoint(): Expected a valid UTF-8 character.');
                break;
            }
    ...
    public static function utf8CharToSurrogatePair($utf8Char)
    {
        ...
        {
            throw new lsException('Zend_Utf8::utf8CharToSurrogatePair(): Expected an extended UTF-8 character.');
        }
        $codePoint -= 0x10000;
    ...
    public static function unescape($value, array $options = array())
    {
        ...
        {
            throw new lsException('Zend_Utf8::unescape(): Expected a valid read (callable, array).');
        }
        ...
        while (preg_match($pattern, $value, $matches, 0, $offset))
        {
            ...
            else
            {
                ...
                if ($options['extendedUseSurrogate'] && (0xD800 <= $unicode && $unicode < 0xDC00))
                {
                    ...
                    if (! $matches[2])
                    {
                        throw new lsException('Zend_Utf8::unescape(): Expected an extended UTF-8 character.');
                    }
                    $offset += strlen($matches[0]);
                    $args = array_splice($matches, 2, count($matches) - 1);
    ...
    public static function utf8CharFromCodePoint($codePoint)
    {
        ...
        {
            //reserved for UTF-16 surrogates
            throw new lsException('Zend_Utf8::utf8CharFromCodePoint(): Expected a valid code point.');
        }
    ...
    public static function utf8CharFromSurrogatePair($surrogatePair)
    {
        ...
        {
            throw new lsException('Zend_Utf8::utf8CharFromSurrogatePair(): Expected an extended UTF-8 character.');
        }
    ...
    protected static function validateFilters($options)
    {
        if (isset($options['filters']))
        {
            ...
            {
                throw new lsException('Zend_Utf8::validateFilters(): Expected valid filters.');
            }
            foreach ($options['filters'] as $key => $value)
            {
                if (! self::isCallable($value))
                {
                    throw new lsException('Zend_Utf8::validateFilters(): Expected a valid "'.str_log($key).'" filter.');
                }
            }
            return true;
        }
...
/*
 * BELOW HERE BE PORTABLE-UTF8 LIBRARY. TO UPDATE THIS LIBRARY, JUST REPLACE
 * FROM BELOW HERE UNTIL THE END OF THE FILE WITH THE UPDATED CODE
 */
