...
function page_get($seoname){
    try{
        ...
        if(!$page){
            throw new lsException('page_get(): Page with seoname "'.str_log($seoname).'" does not exist', 'notfound');
        }

        if($page['status'] !== null){
            throw new lsException('page_get(): Page with seoname "'.str_log($seoname).'" has status "'.str_log($page['status']).'" and cannot be displayed', 'status');
        }

        return $page['data'];

    }catch(Exception $e){
        throw new lsException('page_get(): Failed', $e);
    }
}
...
function page_put($name, $data, $status = null){
    try{
        ...
        throw new lsException('page_put(): Failed', $e);
    }
}
?>
