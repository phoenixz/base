                ...
                if($messages instanceof bException){
                    if($_CONFIG['production']){
                        ...
                        if(preg_match('/^[a-z_]+\(\): /', $message) or preg_match('/PHP ERROR [\d+] /', $message)){
                            ...
                            notify('html_flash/bException', tr('html_flash(): Received bException ":code" with message trace ":trace"', array(':code' => $e->getCode(), ':trace' => $e->getMessages("\n"))), 'developers');
                    ...
                    if($_CONFIG['production']){
                        ...
                        $type     = 'error';
                        $messages = array(tr('Something went wrong, please try again later'));
                        notify('html_flash/Exception', tr('html_flash(): Received PHP exception class ":class" with code ":code" and message ":message"', array(':class' => get_class($e), ':code' => $e->getCode(), ':message' => $e->getMessage())), 'developers');
                    ...
                    if($_CONFIG['production']){
                        $type     = 'error';
                        $messages = array(tr('Something went wrong, please try again later'));
                        notify('html_flash/object', tr('html_flash(): Received PHP object with class ":class" and content ":content"', array(':class' => get_class($e), ':content' => print_r($e->getMessage(), true))), 'developers');
