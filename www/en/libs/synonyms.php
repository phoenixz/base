...
function synonym($source, $params = array()){
    try{
        ...
        if(!is_numeric($params['chance']) or ($params['chance'] < 0) or ($params['chance'] > 100)){
            throw new bException(tr('synonym(): Invalid chance specified, please specify a numeric value in between 0 and 100'), 'invalid');
        }
        ...
        throw new bException(tr('synonym(): Failed'), $e);
    }
}
    ...
    try{
        if(!$data = sql_list('SELECT `synonyms` FROM `synonyms` WHERE `word` = :word', array(':word' => $word))){
            if(!sql_get('SELECT COUNT(`id`) AS count FROM `synonyms`', 'count')){
                throw new bException(tr('synonym_get(): Synonyms table is empty. Please run ./scripts/base/importers/synonyms'), 'empty');
            }
        ...
        throw new bException(tr('synonym_get(): Failed'), $e);
    }
}
...
function synonym_random($count = 1, $nospaces = false){
    try{
        ...
        throw new bException(tr('synonym_random(): Failed'), $e);
    }
}
?>
