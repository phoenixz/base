...
function crypto_add_transaction($transaction, $provider){
    ...
    try{
        ...
        if(empty($transaction['id'])){
             ...
             */
            $dbtransaction = sql_get(' SELECT * FROM `crypto_transactions` WHERE `id` = :id', array(':id' => $transaction['id']));
show($dbtransaction);
            if(empty($dbtransaction)){
                throw new bException(tr('crypto_add_transaction(): Specified transaction ":transaction" does not exist', array(':transaction' => $transaction['id'])), 'not-exist');
            }
