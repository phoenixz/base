...
try{
    ...
    if(isset_get($_POST['doupdate'])){
        ...
        html_flash_set(log_database('Updated password for user "'.str_log(user_name($_SESSION['user'])).'"', 'user_update'), 'success');
    ...
    if(isset_get($_POST['doupdate2'])){
        ...
        /*user_update_pin(array('id'        => $_SESSION['user']['id'],
                                   ...
                                   'cpassword' => isset_get($_POST['cpassword2'])));
        */
        html_flash_set(log_database('Updated pin for user "'.str_log(user_name($_SESSION['user'])).'"', 'user_update'), 'success');
        redirect(domain('/admin/profile.php'));
    }
                ...
                <div class="row">
                    <div class="col-md-12">
                        <section class="panel">
                            ...
                            <div class="panel-body">
                                <div class="form-group">
                                    <label class="col-md-3 control-label" for="cpassword2">'.tr('Current password').'</label>
                                    <div class="col-md-6">
                                        <input type="password" name="cpassword2" id="cpassword2" class="form-control" value="" maxlength="32">
                                    </div>
                                </div>
                                <hr>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input type="number" name="pin" id="pin" class="form-control" value="" length="4">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-md-3 control-label" for="pin2">'.tr('Validate pin').'</label>
                                    <div class="col-md-6">
                                        <input type="number" name="pin2" id="pin2" class="form-control" value="" slength="4">
                                    </div>
                                </div>
...
$vj->validate('cpassword2', 'required' , 'true' , '<span class="FcbErrorTail"></span>'.tr('Please provide your current password'));
$vj->validate('cpassword2', 'minlength', '8'    , '<span class="FcbErrorTail"></span>'.tr('Please ensure that your current password has at least 8 characters'));
$vj->validate('pin'       , 'required' , 'true' , '<span class="FcbErrorTail"></span>'.tr('Please provide the pin code'));
$vj->validate('pin'       , 'minlength', '4'    , '<span class="FcbErrorTail"></span>'.tr('Please ensure that the pin code has 4 digits'));
$vj->validate('pin'       , 'maxlength', '4'    , '<span class="FcbErrorTail"></span>'.tr('Please ensure that the pin code has 4 digits'));
$vj->validate('pin2'      , 'equalTo'  , '#pin' , '<span class="FcbErrorTail"></span>'.tr('The password fields need to be equal'));
...
function s_validate_pin(&$user, $id = null){
    try{
        // Validate input
        $v = new validate_form($user, 'pin,pin2,cpassword2');
    
        $v->hasMinChars ($user['cpassword2'],             8, tr('Please ensure that your current password has a minimum of 8 characters'));
        $v->hasMinChars ($user['pin']       ,             4, tr('Please ensure that the pin has 4 digits'));
        $v->hasMaxChars ($user['pin']       ,             4, tr('Please ensure that the pin has 4 digits'));
